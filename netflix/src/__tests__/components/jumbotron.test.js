...
describe('<Jumbotron />', () => {
  it('renders the <Jumbotron /> with populated data', () => {
    var {container, getByText, getByAltText, getByTestId } = render(
      <Jumbotron.Container>
        {jumboData.map(item => (
  ...
  // it('renders the <Jumbotron /> with no direction', () => {
  //   var {container, getByText, getByAltText, getByTestId } = render(
  //     <Jumbotron.Container>
  //       {jumboData.map(item => (
  //         <Jumbotron key={item.id} data-testid={`${item.id}-direction`}>
  //           <Jumbotron.Pane>
  //             <Jumbotron.Title>{item.title}</Jumbotron.Title>
  //             <Jumbotron.SubTitle>{item.subTitle}</Jumbotron.SubTitle>
  //           </Jumbotron.Pane>
  ...
  //     </Jumbotron.Container>
  //   );
  //         expect(getByTestId('1-direction').props().direction).toBe('row');
  //   expect(container.firstChild).toMatchSnapshot();
  // });
});